...
# ------------------------------------------------------------------
# التشغيل على Render (Web Service) — عبر webhook + منفذ ويب
# ------------------------------------------------------------------
# Render يعطي المنفذ في المتغيّر PORT، ويعطي رابط الخدمة في RENDER_EXTERNAL_URL.
# نستخدمهما ليعمل البوت كخدمة ويب مجانية بدون توقّف.
PORT = int(os.environ.get("PORT", "10000"))
# رابط خدمتك على Render، مثل: https://nafis-bot.onrender.com
# Render يوفّره تلقائيًا في RENDER_EXTERNAL_URL، أو ضعيه يدويًا في WEBHOOK_URL.
...
def main():
    if "ضع_التوكن" in BOT_TOKEN:
        print("⚠️  ضع توكن البوت في متغيّر البيئة BOT_TOKEN على Render.")
        return
    app = build_app()
    if WEBHOOK_URL:
        # وضع الإنتاج على Render: webhook + منفذ ويب
        url = WEBHOOK_URL.rstrip("/") + "/" + BOT_TOKEN
        log.info("تشغيل البوت عبر webhook على المنفذ %s", PORT)
        app.run_webhook(
            listen="0.0.0.0",
            port=PORT,
            url_path=BOT_TOKEN,          # مسار سرّي = التوكن
            webhook_url=url,
            drop_pending_updates=True,
        )
    else:
        # احتياطي للتجربة المحلية على جهازك
        log.info("لا يوجد WEBHOOK_URL — التشغيل محليًا عبر polling.")
